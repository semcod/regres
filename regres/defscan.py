...
def _extract_class_definitions(
    node: ast.ClassDef, path: Path, lines: list[str]
) -> list[Definition]:
...
def _extract_function_definition(
    node, path: Path, lines: list[str], class_method_ids: set[int]
) -> list[Definition]:
...
def extract_go(path: Path) -> list[Definition]:
    ...
    for pattern, kind in [(_GO_STRUCT_RE, "struct"), (_GO_INTERFACE_RE, "interface")]:
        for m in pattern.finditer(text):
            ...
            full = text[
                m.start() : m.start() + text[m.start() :].find(body) + len(body)
            ]
...
def _match_unanchored_pattern(
    name: str, rel: Path, rel_str: str, pat_cmp: str, is_dir: bool
) -> bool:
...
def _path_ignored_by_gitignore(
    path: Path, root: Path, patterns: list[tuple[str, bool]]
) -> bool:
...
def scan(
    ...
    gitignore_root_resolved = (
        gitignore_root.resolve() if gitignore_root is not None else root.resolve()
    )
    ...
    for p in root.rglob("*"):
        if _should_skip_file(
            p, exts, only_within_resolved, gitignore_root_resolved, gitignore_patterns
        ):
            continue
...
def render_text(
    groups: list[tuple[str, list[Definition], list[dict]]],
    root: str,
    min_sim: float = 0.0,
    show_body_lines: int = 6,
) -> str:
    lines = []
    total_defs = sum(len(g[1]) for g in groups)
    lines.append(c(f"{'=' * 70}", BOLD))
    lines.append(c("  defscan — znalezione duplikaty definicji", BOLD))
    ...
    for rank, (name, defs, pairs) in enumerate(groups, 1):
        ...
        for idx, d in enumerate(defs):
            ...
            lines.append(
                f"  [{idx}] {sp}:{d.line_start}–{d.line_end}  {loc_info}{bases_str}"
            )
...
def render_markdown(
    ...
    lines.append(
        f"Grup: {len(groups)}  |  Definicji łącznie: {sum(len(g[1]) for g in groups)}\n"
    )
    lines.append("---\n")
    ...
    for rank, (name, defs, pairs) in enumerate(groups, 1):
        ...
        for idx, d in enumerate(defs):
            ...
            lines.append(
                f"| [{idx}] | `{sp}` | {d.line_start}–{d.line_end} "
                f"| {d.loc} | {bases} |"
            )
        lines.append("")
...
def render_seed_text(
    ...
    lines = []
    lines.append(c(f"{'=' * 70}", BOLD))
    lines.append(c("  defscan — seed similarity (porównanie ciał)", BOLD))
    ...
    for rank, (seed, matches) in enumerate(results, 1):
        ...
        if top_per_seed and len(matches) > top_per_seed:
            lines.append(
                c(f"   … pominięto {len(matches) - top_per_seed} dopasowań", DIM)
            )
        lines.append("")
...
def render_seed_markdown(
    ...
    for rank, (seed, matches) in enumerate(results, 1):
        ...
        for d, simv in shown:
            ...
            sp_d = _short_path(d.path, root)
            same_name = (
                "tak" if d.name.split(".")[-1] == seed.name.split(".")[-1] else "nie"
            )
            lines.append(
...
def render_json(
    groups: list[tuple[str, list[Definition], list[dict]]], root: str
) -> str:
    out = []
    for rank, (name, defs, pairs) in enumerate(groups, 1):
        label, _ = classify_similarity(max((p["similarity"] for p in pairs), default=0))
        out.append(
            {
                "rank": rank,
                "name": name,
...
def _build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--path", "-p", default=".", help="Katalog główny projektu (domyślnie: .)"
    )
    parser.add_argument(
        "--name",
        "-n",
    ...
    parser.add_argument("--json", action="store_true", help="Eksport JSON na stdout")
    parser.add_argument("--md", action="store_true", help="Eksport Markdown na stdout")
    parser.add_argument("--no-color", action="store_true", help="Wyłącz kolory ANSI")
    parser.add_argument(
        "--top", type=int, default=0, help="Pokaż tylko TOP N grup (0 = wszystkie)"
    )
    parser.add_argument(
        "--focus",
        default="",
    ...
    parser.add_argument(
        "--scope", default="", help="Zakres porównania dla --focus (domyślnie: --path)"
    )
    parser.add_argument(
        "--seed",
        default="",
...
def _run_seed_mode(args, root: Path, root_str: str, gitignore_patterns, ext_set):
    ...
    all_defs = _scan_all_definitions(
        scope_path, args, gitignore_patterns, root, ext_set
    )
    seed_defs = _scan_seed_definitions(
        scope_path, seed_path, args, gitignore_patterns, root, ext_set
    )
...
def _print_seed_header(seed_path: Path, scope_path: Path, min_sim: float) -> None:
    ...
    print("Tryb SEED similarity globalna", file=sys.stderr)
...
def _compare_seed_similarity(
    seed_defs: list, all_defs: list, min_sim: float, args
) -> list:
...
def _analyse_and_sort_groups(
    groups_raw: list[tuple[str, list]],
...
def _run_focus_mode(args, root: Path, root_str: str, gitignore_patterns, ext_set):
    ...
    scope_path = Path(args.scope).resolve() if args.scope else root
    print("Tryb FOCUS", file=sys.stderr)
    ...
    if not groups_analysed:
        print("Nie znaleziono grup spełniających kryteria (FOCUS).", file=sys.stderr)
        return

    print(f"Znaleziono {len(groups_analysed)} grup focus↔reszta.", file=sys.stderr)
...
def _run_default_mode(args, root: Path, root_str: str, gitignore_patterns, ext_set):
    ...
    print(
        f"Znaleziono {sum(len(v) for v in index.values())} definicji "
        f"w {len(index)} unikalnych nazwach.",
        file=sys.stderr,
    )

    groups_raw = [
        (name, defs) for name, defs in index.items() if len(defs) >= args.min_count
    ]
