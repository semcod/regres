...
def cmd_find(args, root: Path):
    ...
    if args.json:
        print(
            json.dumps(
                {"word": args.word, "files": results}, indent=2, ensure_ascii=False
            )
        )
        return
    ...
    for r in results:
        print(
            f"{r['path']:<70} {r['word_count']:>5} {r['lines']:>7} {r['size_bytes']:>9}"
        )
...
def cmd_similar(args, root: Path):
    ...
    texts = (
        {k: normalize_code(v, Path(k).suffix) for k, v in texts_raw.items()}
        if normalize
        else texts_raw
    )
    ...
    if args.json:
        print(
            json.dumps(
                {"threshold": threshold, "pairs": pairs}, indent=2, ensure_ascii=False
            )
        )
        return
    if not pairs:
        print(
            f"Nie znaleziono par z podobieństwem ≥ {threshold}%"
...
def _render_cross_lang_symbols(cross_sorted, args):
    ...
    if args.json:
        print(
            json.dumps(
                {"cross_language_symbols": {k: v for k, v in cross_sorted}},
                indent=2,
                ensure_ascii=False,
            )
        )
...
def _render_duplicate_symbols(dups_sorted, args):
    ...
    if args.json:
        print(
            json.dumps(
                {"duplicate_symbols": {k: v for k, v in dups_sorted}},
                indent=2,
                ensure_ascii=False,
            )
        )
...
def cmd_symbols(args, root: Path):
    ...
    if cross_lang:
        ...
        cross_sorted = sorted(
            cross.items(), key=lambda x: len({d["ext"] for d in x[1]}), reverse=True
        )
...
def cmd_dead(args, root: Path):
    ...
    if args.json:
        print(
            json.dumps({"potentially_dead_symbols": dead}, indent=2, ensure_ascii=False)
        )
        return
    if not dead:
...
def _render_hotmap_output(
    ...
    print(
        f"Mapa duplikacji (próg: {threshold}%, {'znorm.' if normalize else 'surowe'}):\n"
    )
    ...
    for h in hotmap:
        ...
        print(
            f"{h['dir']:<55} {h['files']:>6} {h['similar_pairs']:>6} "
...
def _deps_filter_by_word(
    import_map: dict[str, list[str]], word: str
) -> tuple[list, list]:
...
def _format_imports(raw_imports: list) -> str:
    ...
    imports = [
        _sanitize(str(i)).strip() for i in raw_imports if _sanitize(str(i)).strip()
    ]
    if imports:
...
def _toon_files_section(files: list) -> list[str]:
    ...
    for f in files:
        ...
        lines.append(
            f"  {path},{word_count},{lines_count},{size_bytes},{imports_str},{preview_escaped}"
        )
    lines.append("")
    return lines
...
def _find_similar_pairs(texts, normalize, threshold, max_files=200, max_pairs=50):
    ...
    texts_cmp = (
        {k: normalize_code(v, Path(k).suffix) for k, v in texts.items()}
        if normalize
        else texts
    )
            ...
            for j in range(i + 1, len(paths_list)):
                ...
                if sim >= threshold:
                    similar_pairs.append(
                        {"file_a": a, "file_b": b, "similarity": round(sim, 1)}
                    )
        similar_pairs.sort(key=lambda x: x["similarity"], reverse=True)
        similar_pairs = similar_pairs[:max_pairs]
    else:
        similar_pairs = [
            {"note": f"Za dużo plików ({len(paths_list)}) — użyj similar --word"}
        ]
...
def _find_external_importers(files, root, word):
    ...
    for p in iter_files(root):
        ...
        if path_str in source_paths:
            continue
        matched = [
            i for i in extract_imports(read_text(p)) if word.lower() in i.lower()
        ]
        if matched:
...
def _build_report(
    ...
    return {
        ...
        "wrappers": [
            {
                "path": w["path"],
                "score": w["wrapper_score"],
                "reasons": w["wrapper_reasons"],
            }
            for w in wrappers_found
        ],
...
def build_parser():
    ...
    word_arg(pc)
    pc.add_argument(
        "--depth", type=int, default=2, help="Głębokość prefixu (segmenty nazwy)"
    )
    ...
    # symbols  ← NOWY
    psym = sub.add_parser(
        "symbols", help="Indeks symboli: funkcje, klasy, selektory CSS, id HTML"
    )
    word_arg(psym)
    psym.add_argument(
        "--kind",
    ...
    # wrappers  ← NOWY
    pw = sub.add_parser(
        "wrappers", help="Wykryj legacy shims, barrel files, proxy wrappers"
    )
    word_arg(pw)
    pw.add_argument(
        "--min-score",
    ...
    # dead  ← NOWY
    pde = sub.add_parser(
        "dead", help="Symbole zdefiniowane ale prawdopodobnie nieużywane"
    )
    word_arg(pde)
    pde.add_argument(
        "--min-len",
    ...
    pdiff.add_argument(
        "--normalize", action="store_true", help="Usuń komentarze/stringi przed diffem"
    )
    pdiff.add_argument("--context", type=int, default=3, help="Linie kontekstu")

    # hotmap  ← NOWY
    phot = sub.add_parser(
        "hotmap", help="Mapa katalogów wg koncentracji podobnych plików"
    )
    word_arg(phot)
    ...
    pr.add_argument(
        ...
        "-o",
        metavar="FILE",
        help="Plik wyjściowy (domyślnie: refactor-report-WORD.json)",
    )
